...
P_TABLE_GROUPBY:= ' INNER JOIN DM_KHO c ON a.MA = c.MAKHO AND a.MADONVI= c.UNITCODE';
END;
...
P_JOIN_DKIEN := 'a.MADONVI = j.MADONVI AND a.MA = j.MA';
P_TABLE_GROUPBY:= ' INNER JOIN DM_NHOMVATTU c ON a.MA = c.MANHOMVTU  AND a.MADONVI= c.UNITCODE';
END;
...
P_JOIN_DKIEN := 'a.MADONVI = j.MADONVI AND a.MA = j.MA';
P_TABLE_GROUPBY:= ' INNER JOIN DM_LOAIVATTU c ON a.MA = c.MALOAIVATTU AND a.MADONVI= c.UNITCODE ';
END;
...
P_JOIN_DKIEN := 'a.MADONVI = j.MADONVI AND a.MA = j.MA';
P_TABLE_GROUPBY:= '  INNER JOIN DM_KHACHHANG c ON a.MA = c.MAKH AND a.MADONVI= c.UNITCODE';
END;
...
QUERY_STR := 'select a.SOLUONGBAN as SOLUONGBAN,a.VONCHUAVAT as VONCHUAVAT,a.VON as VON,ROUND(a.TIENTHUE) AS TIENTHUE,
                     ROUND(a.DOANHTHU) AS DOANHTHU,a.TONGBAN AS TONGBAN, a.TIENCHIETKHAU AS TIENCHIETKHAU, a.TIENKHUYENMAI AS TIENKHUYENMAI,a.LAIBANLE AS LAIBANLE,
...
INNER JOIN V_VATTU_GIABAN b on b.MAVATTU = ct.MAVATTU AND b.MADONVI= t.MADONVI
WHERE
    t.LOAIGIAODICH = 2

...
SELECT   '||P_SELECT2||',SUM(ct.SOLUONG) as SOLUONG, 
          SUM(ct.SOLUONG*ROUND(NVL(ct.DONGIA,0))) AS TIENHANG '||P_SELECT||' ,
          ROUND(SUM(NVL(ROUND(NVL(ct.DONGIA,0)),0)*NVL(lt.TYGIA/100,0)*ct.SOLUONG)) AS TIENVAT, 
...
WHERE t.TRANGTHAI = 10
    ...
    AND t.NGAYCHUNGTU >=TO_DATE('''||P_TUNGAY||''',''DD/MM/YY'') AND t.UNITCODE = '''||P_UNITCODE||''' AND t.LOAICHUNGTU ='''||P_LOAICHUNGTU||''' '||P_EXPRESSION||' group by '||P_COLUMNS_GROUPBY||' ORDER BY '||P_COLUMNS_GROUPBY||') a';

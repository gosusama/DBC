...
--QUERY FOR KILL SESSION
 ...
 BEGIN
 -- RESULT...
QUERY_STR:= 'SELECT T.UNITCODE, T.MAKHACHHANG, NVL(KH.TENKH,'''') AS TENKH, T.NGAYCT, T.MACHUNGTU, T.TIENTHU, T.TIENBAN, T.TIENNO FROM TEMP_CONGNO T LEFT JOIN DM_KHACHHANG KH ON T.MAKHACHHANG = KH.MAKH AND SUBSTR(T.UNITCODE,0,3) = SUBSTR(KH.UNITCODE,0,3) ORDER BY STT';

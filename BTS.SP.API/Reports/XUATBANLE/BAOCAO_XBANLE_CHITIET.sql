create or replace PROCEDURE BAOCAO_XBANLE_CHITIET(P_GROUPBY IN VARCHAR2, P_MAKHO IN VARCHAR2, P_MALOAI IN VARCHAR2,  
P_MANHOM IN VARCHAR2,P_MAVATTU IN VARCHAR2,P_NHACUNGCAP IN VARCHAR2, P_XUATXU IN VARCHAR2, P_NGUOIDUNG IN VARCHAR2,
...
P_TABLE_GROUPBY:= ' left join DM_KHO c ON a.GROUPCODE = c.MAKHO AND SUBSTR(a.MADONVI,0,3)= SUBSTR(c.UNITCODE,0,3) ';
END;
...
P_TABLE_GROUPBY:= ' left join DM_NHOMVATTU c ON a.GROUPCODE = c.MANHOMVTU AND SUBSTR(a.MADONVI,0,3)= SUBSTR(c.UNITCODE,0,3) ';
END;
...
P_TABLE_GROUPBY:= ' left join DM_LOAIVATTU c ON a.GROUPCODE = c.MALOAIVATTU AND SUBSTR(a.MADONVI,0,3)= SUBSTR(c.UNITCODE,0,3) ';
END;
...
P_TABLE_GROUPBY:= ' left join DM_NHACUNGCAP c ON a.GROUPCODE = c.MANCC AND SUBSTR(a.MADONVI,0,3)= SUBSTR(c.UNITCODE,0,3) ';
END;
...
BEGIN
P_COLUMNS_GROUPBY:= 'b.BARCODE, b.MAVATTU, b.TENVATTU, t.MANGUOITAO,t.NGAYPHATSINH, t.MADONVI';
P_SELECT_COLUMNS_OUT_GROUPBY:= 'a.BARCODE AS BARCODE,a.MA AS MaCon, a.TEN AS TenCon,c.MANHANVIEN AS MaCha, c.TENNHANVIEN AS TenCha,(a.NGAYPHATSINH) AS NgayGiaoDich, (a.MADONVI) AS MADONVI ';
P_SELECT_COLUMNS_IN_GROUPBY:= ' b.BARCODE AS BARCODE, b.MAVATTU AS MA, b.TENVATTU AS TEN, t.MANGUOITAO AS GROUPCODE,t.NGAYPHATSINH AS NGAYPHATSINH, t.MADONVI';
P_TABLE_GROUPBY:= ' left join AU_NGUOIDUNG c ON a.GROUPCODE = c.MANHANVIEN AND SUBSTR(a.MADONVI,0,3)= SUBSTR(c.UNITCODE,0,3) ';
END;
...
WHERE
 ...
 DBMS_OUTPUT.put_line (QUERY_STR );    
  ...
  -- BÁO CÁO THÊM TRƯỜNG HỢP XEM THEO BÓ HÀNG

  create or replace PROCEDURE BAOCAO_XBANLE_CHITIET(P_GROUPBY IN VARCHAR2, P_MAKHO IN VARCHAR2, P_MALOAI IN VARCHAR2,  
P_MANHOM IN VARCHAR2,P_MAVATTU IN VARCHAR2,P_NHACUNGCAP IN VARCHAR2, P_XUATXU IN VARCHAR2, P_NGUOIDUNG IN VARCHAR2,
P_UNITCODE IN VARCHAR2, P_TUNGAY IN DATE,P_DENNGAY IN DATE, P_BOHANG IN VARCHAR2, cur  OUT SYS_REFCURSOR) IS
  P_EXPRESSION VARCHAR(2000);
  QUERY_STR VARCHAR(5000);
  P_SELECT_COLUMNS_IN_GROUPBY VARCHAR(2000);
P_SELECT_COLUMNS_OUT_GROUPBY VARCHAR(2000);
...
P_SELECT_COLUMNS_OUT_GROUPBY:= 'a.BARCODE AS BARCODE,a.MA AS MaCon, a.TEN AS TenCon,c.MANHANVIEN AS MaCha, c.TENNHANVIEN AS TenCha,(a.NGAYPHATSINH) AS NgayGiaoDich, (a.MADONVI) AS MADONVI ';
P_SELECT_COLUMNS_IN_GROUPBY:= ' b.BARCODE AS BARCODE, b.MAVATTU AS MA, b.TENVATTU AS TEN, t.MANGUOITAO AS GROUPCODE,t.NGAYPHATSINH AS NGAYPHATSINH, t.MADONVI';
P_TABLE_GROUPBY:= ' left join AU_NGUOIDUNG c ON a.GROUPCODE = c.MANHANVIEN AND a.MADONVI= c.UNITCODE ';
END;
ELSIF P_GROUPBY = 'BOHANG' THEN
BEGIN
P_COLUMNS_GROUPBY:= 'CT.MABOPK,BH.TENBOHANG,t.NGAYPHATSINH,T.MADONVI';
P_SELECT_COLUMNS_OUT_GROUPBY:= ' a.MA AS MaCon, a.TEN AS TenCon,  a.MA AS MaCha, a.TEN AS TenCha,(a.NGAYPHATSINH) AS NgayGiaoDich, (a.MADONVI) AS MADONVI';
P_SELECT_COLUMNS_IN_GROUPBY:= ' CT.MABOPK AS MA, BH.TENBOHANG AS TEN, t.NGAYPHATSINH AS NGAYPHATSINH, T.MADONVI ';
P_TABLE_GROUPBY:= ' ON a.MA = j.MA ';
END;
ELSE --ELSIF P_GROUPBY = 'MAVATTU' THEN
BEGIN
P_COLUMNS_GROUPBY:= 'b.BARCODE, b.MAVATTU, b.TENVATTU, t.NGAYPHATSINH, t.MADONVI';
...
IF (P_GROUPBY = 'BOHANG' OR (P_GROUPBY = 'BOHANG' AND P_BOHANG IS NOT NULL) OR (P_GROUPBY = 'BOHANG' AND P_BOHANG IS NULL)) THEN
    ...
    SELECT  '||P_SELECT_COLUMNS_IN_GROUPBY||'
                          ...
                          ,SUM( NVL(ct.SOLUONG,0) * NVL(ct.GIABANLECOVAT,0) - NVL(ct.TIENKHUYENMAI, 0) - NVL(ct.TIENVOUCHER, 0)) as TONGBAN
                          ,SUM((NVL(ct.VATBAN,0) / 100) * ((NVL(ct.SOLUONG,0) * NVL(ct.GIABANLECOVAT,0) - NVL(ct.TIENKHUYENMAI, 0) - NVL(ct.TIENVOUCHER, 0)) / (1+ NVL(ct.VATBAN,0) / 100)) ) as TIENTHUE
                          ,SUM(NVL(ct.SOLUONG,0) * NVL(ct.GIABANLECOVAT,0) - NVL(ct.TIENKHUYENMAI, 0) - NVL(ct.TIENVOUCHER, 0)  - NVL(ct.TIENCHIETKHAU, 0) - NVL(ct.GIAVON,0) * NVL(ct.SOLUONG,0)) AS LAIBANLE 
    FROM NVHANGGDQUAY_ASYNCCLIENT ct 
    INNER JOIN NVGDQUAY_ASYNCCLIENT T ON T.MAGIAODICHQUAYPK = CT.MAGDQUAYPK
    ...
    WHERE
        ...
        AND TO_DATE(t.NGAYPHATSINH,''DD/MM/YY'') >= TO_DATE('''||P_TUNGAY||''',''DD/MM/YY'') 
        '||P_EXPRESSION||' 
        GROUP BY '||P_COLUMNS_GROUPBY||' ) j '||P_TABLE_GROUPBY || ' ORDER BY a.MA';
END IF;
 DBMS_OUTPUT.put_line (QUERY_STR );    
  OPEN cur FOR QUERY_STR;
  EXCEPTION
  ...
  END BAOCAO_XBANLE_CHITIET;
 -- END BÁO CÁO XUÁT BÁN LẺ CHI TIẾT
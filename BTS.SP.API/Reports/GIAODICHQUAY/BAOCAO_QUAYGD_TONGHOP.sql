create or replace PROCEDURE "BAOCAO_QUAYGD_TONGHOP" (P_GROUPBY IN VARCHAR2,P_MAKHO IN VARCHAR2,P_MALOAI IN VARCHAR2,  P_MANHOM IN VARCHAR2,P_MAVATTU IN VARCHAR2, P_MAKHACHHANG IN VARCHAR2, P_XUATXU IN VARCHAR2, P_UNITCODE IN VARCHAR2, P_TUNGAY IN DATE, P_DENNGAY IN DATE, CUR  OUT SYS_REFCURSOR) AS
...
P_JOIN_DKIEN := 'a.MADONVI = j.MADONVI AND a.MA = j.MA';
P_TABLE_GROUPBY:= '  LEFT JOIN DM_KHACHHANG c ON a.MA = c.MAKH AND a.MADONVI= c.UNITCODE';
END;
...
QUERY_STR := 'SELECT (a.SOLUONGBAN) as SOLUONGBAN,TO_CHAR(a.VONCHUAVAT) as VONCHUAVAT,(a.VON) as VON,round((a.TIENTHUE)) AS TIENTHUE,

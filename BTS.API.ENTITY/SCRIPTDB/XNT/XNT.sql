...
/*<TOAD_FILE_CHUNK>*/
--------------------------------------------------------
--  File created - Saturday-December-10-2016    V11/12/2016
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package XNT
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE "TBNETERP"."XNT" AS
   ...
   PROCEDURE XNT_CREATE_TABLE_TONKY(P_TABLENAME  IN VARCHAR2, P_UNITCODE  IN VARCHAR2, P_NAM  IN NUMBER,P_KY  IN NUMBER);  
   PROCEDURE XNT_UPDATE_TONDAUKY(P_TABLENAME_KYTRUOC IN VARCHAR2,P_TABLENAME  IN VARCHAR2);    
   PROCEDURE XNT_TANG_TONKY(P_TABLENAME_KYTRUOC IN VARCHAR2, P_TABLENAME IN VARCHAR2, P_MADONVI IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_MANHOMPTNX IN VARCHAR2);
   PROCEDURE XNT_GIAM_TONKY(P_TABLENAME_KYTRUOC IN VARCHAR2 ,P_TABLENAME IN VARCHAR2, P_MADONVI IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_MANHOMPTNX IN VARCHAR2);
...
--------------------------------------------------------
   ...
   PROCEDURE XNT_CREATE_TABLE_TONKY(P_TABLENAME  IN VARCHAR2, P_UNITCODE  IN VARCHAR2, P_NAM  IN NUMBER,P_KY  IN NUMBER) IS
...
PROCEDURE XNT_GIAM_TONKY(P_TABLENAME IN VARCHAR2, P_MADONVI IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_MANHOMPTNX IN VARCHAR2,P_TUNGAY DATE,P_DENNGAY DATE ) IS
  ...
  PROCEDURE XNT_KHOASO(P_TABLENAME_KYTRUOC IN VARCHAR2,P_TABLENAME IN VARCHAR2, P_UNITCODE IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER)IS
    ...
    BEGIN
      ...
      BEGIN
        XNT_CREATE_TABLE_TONKY(P_TABLENAME, P_UNITCODE, P_NAM, P_KY);
        IF TRIM(P_TABLENAME_KYTRUOC) IS NULL THEN
        XNT_CREATE_TONDAUKY(P_TABLENAME, P_UNITCODE, P_NAM, P_KY, 'TDK',P_TUNGAY,P_DENNGAY);
        END IF;
        IF TRIM(P_TABLENAME_KYTRUOC) IS NOT NULL THEN
          BEGIN
            XNT_UPDATE_TONKYTRUOC(P_TABLENAME_KYTRUOC,P_TABLENAME, P_UNITCODE, P_NAM, P_KY);
          END;  
        END IF;
...
QUERY_STR VARCHAR(32767) := 'select a.MAVATTU as Code, b.TENVATTU as Name, b.donvitinh as Unit, 
                                    ...
                                    a.toncuoikysl as ClosingQuangtity, a.toncuoikygt as ClosingValue, a.makho as WareHouseCode, a.unitcode as UnitCode
...
BEGIN
--SELECT KY, NAM INTO KY_BATDAU, NAM_BATDAU FROM DMKYKETOAN WHERE  P_TUNGAY BETWEEN  TUNGAY AND DENNGAY  AND UNITCODE = P_UNITCODE;

SELECT KY, NAM, TUNGAY INTO KY_KETTHUC, NAM_KETTHUC, NGAYBATDAU_CUAKY FROM (SELECT KY, NAM, TUNGAY FROM DMKYKETOAN WHERE  UNITCODE = P_UNITCODE AND TRANGTHAI = 10 AND DENNGAY <= P_DENNGAY ORDER BY DENNGAY DESC) where ROWNUM = 1;


 P_SQL_INSERT := 'INSERT INTO TMP_XTN_NGAY (UNITCODE,MAKHO,MAVATTU, TONCUOIKYSL, TONCUOIKYGT) SELECT 
UNITCODE,MAKHO,MAVATTU, TONDAUKYSL, TONDAUKYGT FROM XNT_'||NAM_KETTHUC||'_KY_'||KY_KETTHUC;
BEGIN
EXECUTE IMMEDIATE P_SQL_INSERT;    
...
QUERY_STR:= 'select a.MAVATTU as Code, b.TENVATTU as Name, b.donvitinh as Unit, 
                                    ...
                                    a.xuatsl as DecreaseQuantity, a.xuatgt as DecreaseValue,
                                    a.toncuoikysl as ClosingQuangtity, a.toncuoikygt as ClosingValue, a.makho as WareHouseCode, a.unitcode as UnitCode
                                    from TMP_XTN_NGAY a left join dmvattu b on a.Mavattu = b.mavattu';
IF TRIM(P_WAREHOUSE) IS NOT NULL THEN
...
EXCEPTION
   WHEN NO_DATA_FOUND
   THEN
      DBMS_OUTPUT.put_line ('<your message>' || P_SQL_INSERT);
   WHEN OTHERS
   THEN
      DBMS_OUTPUT.put_line (QUERY_STR  || SQLERRM);
END BAOCAO_XNT_NGAY;
...
N_SQL_INSERT:='
  ...
  N_COUNT NUMBER;
    ...
    TONDAUKYSL = TONCUOIKYSL - VT_INDEX.SOLUONG,
    TONDAUKYGT = TONCUOIKYGT - VT_INDEX.NHAPGT

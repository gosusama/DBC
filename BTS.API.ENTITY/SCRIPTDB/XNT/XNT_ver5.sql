...
SELECT UNITCODE, NAM, KY, MAKHO, MAVATTU, GIAVON, TONCUOIKYGT, TONCUOIKYSL, TONCUOIKYGT, TONCUOIKYSL FROM '||P_TABLENAME_KYTRUOC||' b WHERE b.UNITCODE = '''|| P_UNITCODE ||'''';
...
BEGIN -- Bản ghi đầu tiên
 P_SQL_INSERT := 'INSERT INTO TMP_XTN_NGAY
         ...
         SELECT UNITCODE, MAKHO, MAVATTU, GIAVON, TONDAUKYSL, TONDAUKYGT, NHAPSL, NHAPGT, XUATSL, XUATGT, TONCUOIKYSL, TONCUOIKYGT
         FROM XNT_'|| NAM_KETTHUC ||'_KY_'||KY_KETTHUC||' WHERE 1=1 ';
IF TRIM(P_WAREHOUSE) IS NOT NULL THEN
P_SQL_INSERT := P_SQL_INSERT||' AND MAKHO IN ('||P_WAREHOUSE||')';
END IF;
IF TRIM(P_MALOAI) IS NOT NULL THEN
P_SQL_INSERT := P_SQL_INSERT||' AND MALOAIVATTU IN ('||P_MALOAI||')';
END IF;
IF TRIM(P_MANHOM) IS NOT NULL THEN
P_SQL_INSERT := P_SQL_INSERT||' AND MANHOMVATTU IN ('||P_MANHOM||')';
END IF;
IF TRIM(P_MAVATTU) IS NOT NULL THEN
P_SQL_INSERT := P_SQL_INSERT||' AND MAVATTU IN ('||P_MAVATTU||')';
END IF;  
execute IMMEDIATE P_SQL_INSERT;
EXCEPTION
            WHEN NO_DATA_FOUND THEN
...
BEGIN --Vòng thứ 2 trở đi
 P_SQL_INSERT := 'INSERT INTO TMP_XTN_NGAY
         ...
         SELECT UNITCODE, MAKHO, MAVATTU, GIAVON, NHAPSL, NHAPGT, XUATSL, XUATGT, 0, 0
         FROM XNT_'|| NAM_KETTHUC ||'_KY_'||KY_KETTHUC||' WHERE 1=1 ';
IF TRIM(P_WAREHOUSE) IS NOT NULL THEN
P_SQL_INSERT := P_SQL_INSERT||' AND MAKHO IN ('||P_WAREHOUSE||')';
END IF;
...
PROCEDURE BAOCAO_XNT_NGAY_THEOKHO(P_WAREHOUSE IN VARCHAR2, P_MALOAI IN VARCHAR2,  P_MANHOM IN VARCHAR2,P_MAVATTU IN VARCHAR2,P_UNITCODE  IN VARCHAR2, P_TUNGAY IN DATE,P_DENNGAY IN DATE, XTNCOLLECTION  OUT SYS_REFCURSOR) IS
KY_KETTHUC NUMBER;
NAM_KETTHUC NUMBER;
QUERY_STR VARCHAR(32767);
P_SQL_INSERT VARCHAR(32767);
...
PROCEDURE BAOCAO_XNT_NGAY_THEOLOAI(P_WAREHOUSE IN VARCHAR2, P_MALOAI IN VARCHAR2,  P_MANHOM IN VARCHAR2,P_MAVATTU IN VARCHAR2,P_UNITCODE  IN VARCHAR2, P_TUNGAY IN DATE,P_DENNGAY IN DATE, XTNCOLLECTION  OUT SYS_REFCURSOR) IS
QUERY_STR VARCHAR(32767);
KY_KETTHUC NUMBER;
NAM_KETTHUC NUMBER;
P_SQL_INSERT VARCHAR(32767);
...
PROCEDURE BAOCAO_XNT_NGAY_THEONHOM(P_WAREHOUSE IN VARCHAR2, P_MALOAI IN VARCHAR2,  P_MANHOM IN VARCHAR2,P_MAVATTU IN VARCHAR2,P_UNITCODE  IN VARCHAR2, P_TUNGAY IN DATE,P_DENNGAY IN DATE, XTNCOLLECTION  OUT SYS_REFCURSOR) IS
QUERY_STR VARCHAR(32767);
KY_KETTHUC NUMBER;
NAM_KETTHUC NUMBER;
P_SQL_INSERT VARCHAR(32767);
...
QUERY_STR:= 'select a.mavattu as Code,
                                    (a.toncuoikysl - a.nhapsl +a.xuatsl)  as OpeningBalanceQuantity,
                                    (a.toncuoikygt - a.nhapgt +a.xuatgt) as OpeningBalanceValue,
                                    a.nhapsl as IncreaseQuantity, a.nhapgt as IncreaseValue,
                                    a.xuatsl as DecreaseQuantity, a.xuatgt as DecreaseValue,
                                    a.toncuoikysl as ClosingQuantity, a.toncuoikygt as ClosingValue
                                    from TMP_XTN_NGAY a left join dmvattu b on a.Mavattu = b.mavattu WHERE 1=1';
OPEN XTNCOLLECTION FOR QUERY_STR;
EXCEPTION
   WHEN NO_DATA_FOUND

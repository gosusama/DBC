...
PROCEDURE CAPNHAT_GIAVON_QUAYGIAODICH(P_TABLENAME IN VARCHAR, P_UNITCODE  IN VARCHAR2);
END XNT;
...
BEGIN
	...
	BEGIN
	STR_IF_REFUND := 'SELECT B.MAVATTU, B.MAKHONHAP, B.UNITCODE,B.SOLUONG AS SOLUONG,CASE  WHEN P.TONCUOIKYSL =  0 THEN 0 WHEN (P.TONDAUKYSL + P.NHAPSL) IS NULL THEN 0 ELSE B.SOLUONG*((P.TONDAUKYGT + P.NHAPGT)/(P.TONDAUKYSL + P.NHAPSL)) END AS NHAPGT FROM
...
PROCEDURE XNT_GIAM_TONKY(P_TABLENAME IN VARCHAR2, P_MADONVI IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_MANHOMPTNX IN VARCHAR2,P_TUNGAY DATE,P_DENNGAY DATE ) IS
  ...
  BEGIN
    ...
    N_COUNT NUMBER :=0; P_TONDAUKYSL NUMBER :=0; P_TONDAUKYGT NUMBER :=0;
    CURSOR NHAP_VATTUGD IS    
	 SELECT B.MAVATTU, B.MAKHOXUAT, B.UNITCODE,B.SOLUONG AS SOLUONG,CASE  WHEN P.TONCUOIKYSL =  0 THEN 0 WHEN (P.TONDAUKYSL + P.NHAPSL) IS NULL THEN 0 ELSE B.SOLUONG*((P.TONDAUKYGT + P.NHAPGT)/(P.TONDAUKYSL + P.NHAPSL)) END AS XUATGT FROM
...
P_SELECT_COLUMNS_GROUPBY:= ' b.MAVATTU AS Code,  b.TENVATTU as TenVT, b.TENVATTU AS Ten,b.BARCODE as BARCODE ';
P_TABLE_GROUPBY:= ' ';
END;
END IF;
...
PROCEDURE XNT_GIAM_PHIEU(P_TABLENAME IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_ID IN VARCHAR2 ) IS
  ...
  N_SQL_INSERT:='DECLARE 
    N_COUNT NUMBER :=0; P_TONDAUKYSL NUMBER :=0; P_TONDAUKYGT NUMBER :=0;
    CURSOR NHAP_VATTUGD IS 
	SELECT B.MAVATTU, B.MAKHOXUAT, B.UNITCODE, B.SOLUONG, CASE  WHEN P.TONCUOIKYSL =  0 THEN 0 WHEN (P.TONDAUKYSL + P.NHAPSL) IS NULL THEN 0 ELSE B.SOLUONG*((P.TONDAUKYGT + P.NHAPGT)/(P.TONDAUKYSL + P.NHAPSL)) END AS XUATGT from 
  ...
  PROCEDURE XNT_TANG_PHIEU(P_TABLENAME IN VARCHAR2, P_NAM IN VARCHAR2,P_KY IN NUMBER,P_ID IN VARCHAR2 ) IS
	...
	STR_IF_REFUND VARCHAR(32767);
BEGIN
SELECT LOAICHUNGTU INTO N_LOAICHUNGTU FROM VIEW_VATTUGD_XNT WHERE ID = P_ID;
	STR_IF_REFUND:= 'SELECT MAVATTU,MAKHONHAP,UNITCODE,SUM(SOLUONG) AS SOLUONG,SUM(SOLUONG*DONGIA) AS NHAPGT FROM VIEW_VATTUGD_XNT V WHERE 
    ID='''||P_ID||'''     
    GROUP BY MAVATTU,MAKHONHAP,UNITCODE;';
	IF N_LOAICHUNGTU = '2' THEN
	BEGIN
	STR_IF_REFUND := 'SELECT B.MAVATTU, B.MAKHONHAP, B.UNITCODE, B.SOLUONG, CASE  WHEN P.TONCUOIKYSL =  0 THEN 0 WHEN (P.TONDAUKYSL + P.NHAPSL) IS NULL THEN 0 ELSE B.SOLUONG*((P.TONDAUKYGT + P.NHAPGT)/(P.TONDAUKYSL + P.NHAPSL)) END AS NHAPGT from 
...
PROCEDURE CAPNHAT_GIAVON_QUAYGIAODICH(P_TABLENAME IN VARCHAR, P_UNITCODE  IN VARCHAR2) IS
...
(select UNITCODE, MAKHO, MAVATTU,SUM(case (TONDAUKYSL + NHAPSL) when 0 then 0 else 
(TONDAUKYGT + NHAPGT)/(TONDAUKYSL + NHAPSL) end)  as DONGIA from '||P_TABLENAME||' group by UNITCODE, MAKHO, MAVATTU) b
ON (a.MAKHOHANG = b.MAKHO and a.MAVATTU = b.MAVATTU)
WHEN MATCHED THEN
  UPDATE SET a.GIAVON = b.DONGIA
';
BEGIN

...
BEGIN
	...
	BEGIN
	STR_IF_REFUND := 'SELECT B.MAVATTU, B.MAKHONHAP, B.UNITCODE,B.SOLUONG AS SOLUONG,CASE  WHEN P.TONCUOIKYSL =  0 THEN NVL(G.GIAMUA, 0)*B.SOLUONG WHEN (P.TONDAUKYSL + P.NHAPSL) IS NULL THEN NVL(G.GIAMUA, 0)*B.SOLUONG  ELSE ABS(B.SOLUONG*((P.TONDAUKYGT + P.NHAPGT)/(P.TONDAUKYSL + P.NHAPSL))) END AS NHAPGT FROM
...
PROCEDURE XNT_GIAM_TONKY(P_TABLENAME IN VARCHAR2, P_MADONVI IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_MANHOMPTNX IN VARCHAR2,P_TUNGAY DATE,P_DENNGAY DATE ) IS
  ...
  BEGIN
    ...
    N_COUNT NUMBER :=0; P_TONDAUKYSL NUMBER :=0; P_TONDAUKYGT NUMBER :=0;
    CURSOR NHAP_VATTUGD IS    
	 SELECT B.MAVATTU, B.MAKHOXUAT, B.UNITCODE,B.SOLUONG AS SOLUONG,CASE  WHEN P.TONCUOIKYSL =  0 THEN NVL(G.GIAMUA, 0)*B.SOLUONG WHEN (P.TONDAUKYSL + P.NHAPSL) IS NULL THEN NVL(G.GIAMUA, 0)*B.SOLUONG  ELSE ABS(B.SOLUONG*((P.TONDAUKYGT + P.NHAPGT)/(P.TONDAUKYSL + P.NHAPSL))) END AS XUATGT FROM
...
PROCEDURE XNT_GIAM_PHIEU(P_TABLENAME IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_ID IN VARCHAR2 ) IS
  ...
  N_SQL_INSERT:='DECLARE 
    N_COUNT NUMBER :=0; P_TONDAUKYSL NUMBER :=0; P_TONDAUKYGT NUMBER :=0;
    CURSOR NHAP_VATTUGD IS 
	SELECT B.MAVATTU, B.MAKHOXUAT, B.UNITCODE, B.SOLUONG, CASE  WHEN P.TONCUOIKYSL =  0 THEN NVL(G.GIAMUA, 0)*B.SOLUONG WHEN (P.TONDAUKYSL + P.NHAPSL) IS NULL THEN NVL(G.GIAMUA, 0)*B.SOLUONG  ELSE ABS(B.SOLUONG*((P.TONDAUKYGT + P.NHAPGT)/(P.TONDAUKYSL + P.NHAPSL))) END AS XUATGT from 
...
(select X.UNITCODE as UNITCODE, X.MAKHO as MAKHO, X.MAVATTU AS MAVATTU,SUM(case (X.TONDAUKYSL + X.NHAPSL) when 0 then 0 else 
(X.TONDAUKYGT + X.NHAPGT)/(X.TONDAUKYSL + X.NHAPSL) end)  as DONGIA from '||P_TABLENAME||' X
group by X.UNITCODE, X.MAKHO, X.MAVATTU) b
ON (a.MAKHOHANG = b.MAKHO and a.MAVATTU = b.MAVATTU and b.UNITCODE = '''||P_UNITCODE||''')
WHEN MATCHED THEN

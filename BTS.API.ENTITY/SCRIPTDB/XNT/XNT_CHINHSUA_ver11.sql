...
SELECT kkct.ID AS ID, TO_NCHAR(kkct.MADONVI) AS UNITCODE,kk.TRANGTHAI AS TRANGTHAI,
    TO_NCHAR(SUBSTR(kk.MAPHIEUKIEMKE,0,4)) AS LOAICHUNGTU, TO_DATE(kk.NGAYKIEMKE,'DD/MM/YY') AS NGAYCHUNGTU,
...
--Edit: Nguyen Quang Huy: Update Gia Von GDQ, PHAMTUANANH tinh gia von binh quan, kiem ke hang hoa
...
PROCEDURE XNT_GIAM_PHIEU(P_TABLENAME IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_ID IN VARCHAR2 ) IS
  ...
  N_SQL_INSERT:='DECLARE 
	...
	CASE  WHEN P.GIAVON =  0 THEN NVL(G.GIAMUA, 0)*B.SOLUONG 
	ELSE P.GIAVON END AS XUATGT from 
...
and (select t.NGAYCHUNGTU from VATTUCHUNGTU t where t.MACHUNGTUPK = a.MACHUNGTUPK) = TO_DATE('''|| P_NGAYPHATSINH ||''',''DD/MM/YY'') 

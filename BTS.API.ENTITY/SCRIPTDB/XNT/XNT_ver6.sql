...
BEGIN
	...
	CASE  WHEN P.GIAVON IS NULL || P.GIAVON = 0 THEN NVL(G.GIAMUA, 0)*B.SOLUONG 
    ...
    LEFT JOIN '||P_TABLENAME||' P  ON  B.MAVATTU = P.MAVATTU AND B.MAKHONHAP = P.MAKHO AND SUBSTR(B.UNITCODE,0,3) = SUBSTR(P.UNITCODE,0,3) 
  ...
  BEGIN FOR VT_INDEX IN NHAP_VATTUGD LOOP
    ...
    BEGIN
      IF(N_COUNT=0) THEN 
        BEGIN 
      INSERT INTO '||P_TABLENAME||' (UNITCODE,NAM,KY,MAKHO,MAVATTU) 
      SELECT VT_INDEX.UNITCODE AS UNITCODE,'|| P_NAM ||' AS NAM,'|| P_KY ||' AS KY,VT_INDEX.MAKHONHAP AS MAKHO,VT_INDEX.MAVATTU AS MAVATTU FROM DMVATTU WHERE UNITCODE LIKE SUBSTR(VT_INDEX.UNITCODE,0) || ''%'' AND MAVATTU=VT_INDEX.MAVATTU;
      COMMIT;
        END;
       END IF;
     END;
...
BEGIN -- Bản ghi đầu tiên
 P_SQL_INSERT := 'INSERT INTO TMP_XTN_NGAY
         ...
         FROM XNT_'|| NAM_KETTHUC ||'_KY_'||KY_KETTHUC||' a left join dmvattu b on a.Mavattu = b.mavattu WHERE 1=1 AND SUBSTR(b.UNITCODE,0,3) = SUBSTR('''||P_UNITCODE||''',0,3)
		 ';
...
P_TABLE_GROUPBY:= ' left join DMNHOMVATTU c ON b.MANHOMVATTU = c.MANHOMVTU AND SUBSTR(c.UNITCODE,0,3) = SUBSTR('''||P_UNITCODE||''',0,3)';
END;
...
P_TABLE_GROUPBY:= ' left join DMLOAIVATTU c ON b.MALOAIVATTU = c.MALOAIVATTU AND SUBSTR(c.UNITCODE,0,3) = SUBSTR('''||P_UNITCODE||''',0,3)';
END;
...
QUERY_STR:= 'select '||P_SELECT_COLUMNS_GROUPBY||', 
                                    ...
                                    from TMP_XTN_NGAY a 
                                    left join dmvattu b on a.Mavattu = b.mavattu AND SUBSTR(b.UNITCODE,0,3) = SUBSTR('''||P_UNITCODE||''',0,3)
                                    '||P_TABLE_GROUPBY||'
                                    WHERE 1=1 
									'||P_EXPRESSION||'
...
P_TABLE_GROUPBY:= ' left join DMKHACHHANG c ON b.MAKHACHHANG = c.MAKH AND SUBSTR(c.UNITCODE,0,3) = SUBSTR('''||P_UNITCODE||''',0,3)';
P_EXPRESSION:= ' AND c.LOAIKHACHHANG = 1';
END;
ELSE --ELSIF P_GROUPBY = 'MAVATTU' THEN
BEGIN
...
QUERY_STR:= 'select '||P_SELECT_COLUMNS_GROUPBY||',
                              ...
                              SUM(a.tondaukygt - a.xuatgt + a.nhapgt) as ClosingValue
                                    ...
                                    '||P_TABLE_GROUPBY||'
                                    WHERE 1=1  AND SUBSTR(b.UNITCODE,0,3) = SUBSTR('''||P_UNITCODE||''',0,3) and (a.tondaukysl * a.tondaukysl) + (a.nhapsl * a.nhapsl) + (a.xuatsl * a.xuatsl) + (a.toncuoikysl* a.toncuoikysl)  > 0 
									'||P_EXPRESSION||'
...
PROCEDURE XNT_GIAM_PHIEU(P_TABLENAME IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_ID IN VARCHAR2 ) IS
  ...
  N_SQL_INSERT:='DECLARE 
    ...
    LEFT JOIN '||P_TABLENAME||' P  ON  B.MAVATTU = P.MAVATTU AND B.MAKHOXUAT = P.MAKHO AND B.UNITCODE = P.UNITCODE 
   	LEFT JOIN DM_VATTU_GIACA G ON B.MAVATTU = G.MAVATTU AND SUBSTR(B.UNITCODE,0,3) = SUBSTR(G.MADONVI,0,3) ;
  BEGIN FOR VT_INDEX IN NHAP_VATTUGD LOOP
    N_COUNT :=0;
      BEGIN 
        SELECT COUNT(*) INTO N_COUNT FROM '||P_TABLENAME||' WHERE 
    ...
    BEGIN
      ...
      SELECT VT_INDEX.UNITCODE AS UNITCODE,'|| P_NAM ||' AS NAM,'|| P_KY ||' AS KY,VT_INDEX.MAKHOXUAT AS MAKHO,VT_INDEX.MAVATTU AS MAVATTU,0 AS XUATSL, 0 AS XUATGT,0 AS TONCUOIKYSL,0 AS TONCUOIKYGT FROM DMVATTU WHERE UNITCODE LIKE SUBSTR(VT_INDEX.UNITCODE,0,3) || ''%'' AND MAVATTU=VT_INDEX.MAVATTU;
      COMMIT;
        END;
       END IF;
     END;
  ...
  BEGIN FOR VT_INDEX IN NHAP_VATTUGD LOOP
    ...
    BEGIN
      IF(N_COUNT=0) THEN 
        BEGIN 
      INSERT INTO '||P_TABLENAME||' (UNITCODE,NAM,KY,MAKHO,MAVATTU) 
      SELECT VT_INDEX.UNITCODE AS UNITCODE,'|| P_NAM ||' AS NAM,'|| P_KY ||' AS KY,VT_INDEX.MAKHONHAP AS MAKHO,VT_INDEX.MAVATTU AS MAVATTU FROM DMVATTU WHERE UNITCODE LIKE SUBSTR(VT_INDEX.UNITCODE,0,3) || ''%'' AND MAVATTU=VT_INDEX.MAVATTU;
      COMMIT;
        END;
       END IF;
     END;

create or replace PROCEDURE            "BAOCAO_XNTNEW_TONGHOP" (P_GROUPBY IN VARCHAR2, P_WAREHOUSE IN VARCHAR2, P_MALOAI IN VARCHAR2,  P_MANHOM IN VARCHAR2,P_MAVATTU IN VARCHAR2, P_MAKHACHHANG IN VARCHAR2,P_UNITCODE  IN VARCHAR2, P_TUNGAY IN DATE,P_DENNGAY IN DATE, XTNCOLLECTION  OUT SYS_REFCURSOR) 
IS
    KY_BATDAU NUMBER;
    NAM_BATDAU NUMBER;
    KY_KETTHUC NUMBER;
...
--LẤY TOÀN BỘ DỮ LIỆU NHẬP-XUẤT
   -- NHẬP MUA, XUẤT BÁN LẺ WEB, NHẬP HÀNG BÁN TRẢ LẠI
    P_SQL_INSERT := 'INSERT INTO XUATNHAPTON_TEMP (MAVATTU, MALOAIVATTU, MANHOMVATTU,MANHACUNGCAP,MAKHO,SOLUONG,DONGIA,THANHTIEN,UNITCODE, LOAICHUNGTU,NGAYCHUNGTU,NGAYDUYETPHIEU)
         SELECT VATTUCHUNGTUCHITIET.MAHANG AS MAVATTU,
          DMVATTU.MALOAIVATTU AS MALOAIVATTU,
          DMVATTU.MANHOMVATTU AS  MANHOMVATTU,
          DMVATTU.MAKHACHHANG AS MANHACUNGCAP,
          ...
          FROM VATTUCHUNGTU
              ...
              INNER JOIN DMVATTU ON VATTUCHUNGTUCHITIET.MAHANG = DMVATTU.MAVATTU AND DMVATTU.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%''
        WHERE VATTUCHUNGTU.LOAICHUNGTU IN (''NMUA'',''NHBANTL'') AND VATTUCHUNGTU.TRANGTHAI = 10 AND VATTUCHUNGTU.UNITCODE = '''||P_UNITCODE||''' AND TO_DATE(VATTUCHUNGTU.NGAYDUYETPHIEU,''DD/MM/YY'') BETWEEN TO_DATE('''||P_TUNGAY||''', ''DD/MM/YY'') AND TO_DATE('''||P_DENNGAY||''', ''DD/MM/YY'')';        
            ...
            P_SQL_INSERT := P_SQL_INSERT||' AND DMVATTU.MALOAIVATTU IN ('||P_MALOAI||')';
            END IF;
            IF TRIM(P_MANHOM) IS NOT NULL THEN
            P_SQL_INSERT := P_SQL_INSERT||' AND DMVATTU.MANHOMVATTU IN ('||P_MANHOM||')';
            END IF;
            ...
            P_SQL_INSERT := P_SQL_INSERT||' AND DMVATTU.MAKHACHHANG IN ('||P_MAKHACHHANG||')';
            END IF;
        --XUẤT BÁN LẺ WEB
      P_SQL_INSERT := P_SQL_INSERT || ' UNION ALL
        ...
        WHERE VATTUCHUNGTU.LOAICHUNGTU=''XBANLE'' AND VATTUCHUNGTU.TRANGTHAI = 10 AND VATTUCHUNGTU.UNITCODE = '''||P_UNITCODE||''' AND TO_DATE(VATTUCHUNGTU.NGAYDUYETPHIEU,''DD/MM/YY'') BETWEEN TO_DATE('''||P_TUNGAY||''', ''DD/MM/YY'') AND TO_DATE('''||P_DENNGAY||''', ''DD/MM/YY'')';        
            IF TRIM(P_WAREHOUSE) IS NOT NULL THEN
            P_SQL_INSERT := P_SQL_INSERT||' AND VATTUCHUNGTU.MAKHOXUAT IN ('||P_WAREHOUSE||')';
            END IF;
            IF TRIM(P_MALOAI) IS NOT NULL THEN
            P_SQL_INSERT := P_SQL_INSERT||' AND DMVATTU.MALOAIVATTU IN ('||P_MALOAI||')';
            END IF;
        ...
        UNION ALL
           ...
           UNION ALL
            ...
            FROM NVGDQUAY_ASYNCCLIENT gdquay
              ...
              INNER JOIN DMVATTU ON hanggdquay.MAVATTU = DMVATTU.MAVATTU  AND DMVATTU.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%''
            ...
            ON (kiemke.MAPHIEUKIEMKE = kiemkechitiet.MAPHIEUKIEMKE)
              INNER JOIN DMVATTU ON kiemkechitiet.MAVATTU = DMVATTU.MAVATTU AND DMVATTU.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%''
        ...
        END;--End Tìm ky
         ...
         P_SQL_INSERT := P_SQL_INSERT || '
            ...
            INNER JOIN DMVATTU ON TDK.MAVATTU = DMVATTU.MAVATTU AND DMVATTU.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%''
            WHERE TDK.UNITCODE = '''||P_UNITCODE||''' AND TDK.TONDAUKYSL <> 0 ';
            IF TRIM(P_WAREHOUSE) IS NOT NULL THEN
            P_SQL_INSERT := P_SQL_INSERT||' AND TDK.MAKHO IN ('||P_WAREHOUSE||')';
            END IF;
            IF TRIM(P_MALOAI) IS NOT NULL THEN
            P_SQL_INSERT := P_SQL_INSERT||' AND DMVATTU.MALOAIVATTU IN ('||P_MALOAI||')';
            END IF;
        ...
        P_TABLE_GROUPBY:= ' LEFT JOIN DMKHO F ON X.MAKHO = F.MAKHO AND F.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%'' ';
        ...
        P_TABLE_GROUPBY:= ' LEFT JOIN DMNHOMVATTU F ON X.MANHOMVATTU = F.MANHOMVTU AND F.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%'' ';
        ...
        P_TABLE_GROUPBY:= ' LEFT JOIN DMLOAIVATTU F ON X.MALOAIVATTU = F.MALOAIVATTU AND F.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%'' ';
        P_ORDERBY := 'X.MALOAIVATTU, F.TENLOAIVT';
        END;

        ELSIF P_GROUPBY = 'MAKHACHHANG' THEN
        BEGIN
        P_SELECT_COLUMNS_GROUPBY:= 'X.MANHACUNGCAP AS MA,F.TENKH AS TEN';
        P_TABLE_GROUPBY:= 'LEFT JOIN DMKHACHHANG F ON X.MANHACUNGCAP = F.MAKH AND F.LOAIKHACHHANG = 1 AND F.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%''';   
        P_COLUMNS_GROUPBY:= 'X.MANHACUNGCAP,F.TENKH';
        P_ORDERBY := 'X.MANHACUNGCAP,F.TENKH';
        END;
        ELSE 
        BEGIN
        P_COLUMNS_GROUPBY:= 'X.MAVATTU,F.TENVATTU';
        P_SELECT_COLUMNS_GROUPBY:= 'X.MAVATTU AS MA,F.TENVATTU AS TEN';
        P_TABLE_GROUPBY:= 'LEFT JOIN DMVATTU F ON X.MAVATTU = F.MAVATTU AND F.UNITCODE LIKE SUBSTR('''||P_UNITCODE||''',0,3) || ''%''';   

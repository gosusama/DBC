...
P_TABLE_GROUPBY:= 'inner join Dmkhachhang kh on A.Makhachhang = kh.Makh';
P_SELECT2:='';
...
QUERY_STR:= 'SELECT b.MAHANG as MAHANG,b.TENHANG as TENHANG, SUM(b.SOLUONG) as SOLUONG, SUM(B.Dongia) as DONGIA '||P_SELECT_COLUMNS_GROUPBY||'
FROM Nvdathang a
inner join Nvdathangchitiet b on A.Sophieupk = B.Sophieupk
'||P_TABLE_GROUPBY||'
WHERE  A.Unitcode = '''||P_UNITCODE||'''

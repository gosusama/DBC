...
BEGIN
	...
	LEFT JOIN DM_VATTU_GIACA G ON B.MAVATTU = G.MAVATTU AND B.UNITCODE = G.MADONVI AND SUBSTR(B.UNITCODE,0,3) = SUBSTR(P.UNITCODE,0,3);
	';
	END;
	END IF;
...
BEGIN--Tìm kỳ
SELECT KY, NAM INTO KY_KETTHUC, NAM_KETTHUC FROM DMKYKETOAN WHERE to_date(DENNGAY, 'dd/MM/yyyy') = to_date(cur_period.today, 'dd/MM/yyyy') AND UNITCODE LIKE '%'||P_UNITCODE||'%' AND TRANGTHAI = 10 AND rownum =1;
   EXCEPTION
...
BEGIN -- Bản ghi đầu tiên
 P_SQL_INSERT := 'INSERT INTO TMP_XTN_NGAY
         ...
         FROM XNT_'|| NAM_KETTHUC ||'_KY_'||KY_KETTHUC||' a left join dmvattu b on a.Mavattu = b.mavattu  AND SUBSTR(b.UNITCODE,0,3) = SUBSTR(a.UNITCODE,0,3)
...
END IF;
END; -- Kết thúc thực thi nhiệm vụ
...
P_TABLE_GROUPBY:= ' left join SYS_DONVI c ON c.MADONVI = a.UNITCODE ';
END;
...
P_TABLE_GROUPBY:= ' left join DMKHO c ON a.MAKHO = c.MAKHO AND SUBSTR(c.UNITCODE,0,3) = SUBSTR(a.UNITCODE,0,3)';
END;
...
P_TABLE_GROUPBY:= ' left join DMNHOMVATTU c ON b.MANHOMVATTU = c.MANHOMVTU AND SUBSTR(c.UNITCODE,0,3) = SUBSTR(a.UNITCODE,0,3)';
END;
...
P_TABLE_GROUPBY:= ' left join DMLOAIVATTU c ON b.MALOAIVATTU = c.MALOAIVATTU AND SUBSTR(c.UNITCODE,0,3) = SUBSTR(a.UNITCODE,0,3)';
END;
...
P_TABLE_GROUPBY:= ' left join DMSUPPLIER c ON b.MAKHACHHANG = c.MANCC AND SUBSTR(c.UNITCODE,0,3) = SUBSTR(a.UNITCODE,0,3)';
P_EXPRESSION:= '';
END;
...
QUERY_STR:= 'select '||P_SELECT_COLUMNS_GROUPBY||', 
                                    ...
                                    from TMP_XTN_NGAY a 
                                    left join dmvattu b on a.Mavattu = b.mavattu AND SUBSTR(b.UNITCODE,0,3) = SUBSTR(a.UNITCODE,0,3)
                                    '||P_TABLE_GROUPBY||'
                                    WHERE 1=1 
									'||P_EXPRESSION||'
...
P_TABLE_GROUPBY:= ' left join SYS_DONVI c ON a.UNITCODE = c.MADONVI';
END;
ELSIF P_GROUPBY = 'MAKHO' THEN
BEGIN

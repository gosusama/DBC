...
BEGIN
  ...
  N_SQL_INSERT:='DECLARE 
    N_COUNT NUMBER :=0; P_TONDAUKYSL NUMBER :=0; P_TONDAUKYGT NUMBER :=0;N_GIATRI_TINHGIAVON_Z  NUMBER :=0;
    CURSOR NHAP_VATTUGD IS 
	'||STR_IF_REFUND||'

  BEGIN FOR VT_INDEX IN NHAP_VATTUGD LOOP
    N_COUNT :=0;
    --CHECK PHƯƠNG THỨC TÍNH GIÁ VỐN THEO MÃ Z TRONG BẢNG THAMSOHETHONG
      SELECT GIATRI_THAMSO INTO N_GIATRI_TINHGIAVON_Z FROM AU_THAMSOHETHONG WHERE MA_THAMSO = ''CACULATOR_Z'' AND UNITCODE = '''||P_MADONVI||''';
      BEGIN 
    ...
    BEGIN
      IF(N_COUNT=0) THEN 
        BEGIN 
      INSERT INTO '||P_TABLENAME||' (UNITCODE,NAM,KY,MAKHO,MAVATTU) 
      SELECT VT_INDEX.UNITCODE AS UNITCODE,'|| P_NAM ||' AS NAM,'|| P_KY ||' AS KY,VT_INDEX.MAKHONHAP AS MAKHO,VT_INDEX.MAVATTU AS MAVATTU FROM DM_VATTU WHERE UNITCODE LIKE SUBSTR(VT_INDEX.UNITCODE,0,3) || ''%'' AND MAVATTU=VT_INDEX.MAVATTU;
      COMMIT;
        END;
       END IF;
     END;
    ...
    BEGIN 
      IF N_GIATRI_TINHGIAVON_Z = 0 THEN
        ...
        IF N_GIATRI_TINHGIAVON_Z = 1 THEN
            IF VT_INDEX.MAVATTU LIKE ''Z%'' THEN 
                UPDATE '||P_TABLENAME||' SET
                  NHAPSL=NHAPSL+VT_INDEX.SOLUONG, 
                  NHAPGT=NHAPGT+VT_INDEX.NHAPGT, 
                  TONCUOIKYSL=TONCUOIKYSL+VT_INDEX.SOLUONG, 
                  TONCUOIKYGT=TONCUOIKYGT+VT_INDEX.NHAPGT,
                  GIAVON =  NVL((SELECT GIAMUA FROM DM_VATTU_GIACA WHERE MAVATTU = VT_INDEX.MAVATTU AND MADONVI = VT_INDEX.UNITCODE), 0)
                  WHERE UNITCODE = VT_INDEX.UNITCODE AND  MAVATTU = VT_INDEX.MAVATTU AND MAKHO= VT_INDEX.MAKHONHAP; 
                COMMIT;
            ELSE
                UPDATE '||P_TABLENAME||' SET
...
PROCEDURE XNT_GIAM_TONKY(P_TABLENAME IN VARCHAR2, P_MADONVI IN VARCHAR2,P_NAM IN VARCHAR2,P_KY IN NUMBER,P_MANHOMPTNX IN VARCHAR2,P_TUNGAY DATE,P_DENNGAY DATE ) IS
  ...
  BEGIN FOR VT_INDEX IN NHAP_VATTUGD LOOP
    ...
    BEGIN
      ...
      SELECT VT_INDEX.UNITCODE AS UNITCODE,'|| P_NAM ||' AS NAM,'|| P_KY ||' AS KY,VT_INDEX.MAKHOXUAT AS MAKHO,VT_INDEX.MAVATTU AS MAVATTU, 0 AS XUATSL, 0 AS XUATGT, 0 AS TONCUOIKYSL, 0 AS TONCUOIKYGT FROM DM_VATTU WHERE UNITCODE LIKE SUBSTR(VT_INDEX.UNITCODE,0,3) || ''%'' AND MAVATTU=VT_INDEX.MAVATTU;
      COMMIT;
        END;
       END IF;
     END;
...
BEGIN -- Báº£n ghi Ä‘áº§u tiÃªn
 P_SQL_INSERT := 'INSERT INTO TMP_XTN_NGAY
         ...
         FROM XNT_'|| NAM_KETTHUC ||'_KY_'||KY_KETTHUC||' a left join DM_VATTU b on a.Mavattu = b.mavattu  AND SUBSTR(b.UNITCODE,0,3) = SUBSTR(a.UNITCODE,0,3)
...
QUERY_STR:= 'select '||P_SELECT_COLUMNS_GROUPBY||', 
                                    ...
                                    from TMP_XTN_NGAY a 
                                    left join DM_VATTU b on a.Mavattu = b.mavattu AND SUBSTR(b.UNITCODE,0,3) = SUBSTR(a.UNITCODE,0,3)
                                    '||P_TABLE_GROUPBY||'
                                    WHERE 1=1 
									'||P_EXPRESSION||'
